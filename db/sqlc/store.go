...
// Store provides all function to execute queries and transaction
type Store struct {
	*Queries
	db *sql.DB
}
...
func (store *Store) TransferTx(
	...
	err := store.execTx(ctx, func(q *Queries) error {
		...
		result.FromEntry, err = q.CreateEntry(
			ctx,
			CreateEntryParams{AccountID: arg.FromAccountID, Amount: -arg.Amount},
		)
		if err != nil {
			return err
		}
		...
		// to update balance in consistent order
...
func transferMoney(
	...
	account1, err = q.AddAccountBalance(
		ctx,
		AddAccountBalanceParams{Amount: amount1, ID: fromAccountID},
	)
	if err != nil {
		return
	}
	account2, err = q.AddAccountBalance(
		ctx,
		AddAccountBalanceParams{Amount: amount2, ID: toAccountID},
	)
	if err != nil {
		return
	}
